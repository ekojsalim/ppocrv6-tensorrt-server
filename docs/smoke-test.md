...
Use this checklist to validate a new checkout with public model files. It
prepares local artifacts, builds the native TensorRT components, starts the
server, and sends generated sample images through the HTTP API.

The generated sample images are smoke fixtures, not accuracy benchmarks. They
include portrait and landscape pages so detector profile coverage is checked
end to end.

## 1. Install Python Helpers
...
```text
artifacts/ppocrv6-medium/source/
artifacts/ppocrv6-medium/derived/rec-hidden.onnx
artifacts/ppocrv6-medium/derived/classifier.npz
artifacts/ppocrv6-medium/classifier/
```

## 3. Build The TensorRT/OpenCV Image

The commands below use host networking and port `8184`. Choose another port if
that is already in use on your machine.
...
Engine builds use the GPU and may require several GiB of available device
memory.

Build the multi-profile hidden recognizer engine:
